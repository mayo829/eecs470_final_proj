...
export CLOCK_PERIOD = 12.87

# the Verilog Compiler command and arguments
VCS =  vcs -sverilog -xprop=tmerge +vc -Mupdate -Mdir=build/csrc -line -full64 -kdb -lca -nc \
      -debug_access+all+reverse $(VCS_BAD_WARNINGS) +define+CLOCK_PERIOD=$(CLOCK_PERIOD) +incdir+verilog/ +memcbk
...
MODULES = cpu mult rob rs BP BTB PHT RAS pre_decode FIFO if_stage s_PhysicalRegisterFile stage_issue stage_ex freeList s_RegisterAliasTable s_ArchitecturalMap stage_complete stage_retire integrated_retirement_pipeline bp2b load SQ dcache
...
BP_FILES = verilog/sys_defs.svh verilog/BHT.sv verilog/BTB.sv verilog/PHT.sv verilog/RAS.sv verilog/pre_decode.sv
build/BP.simv: $(BP_FILES)
build/BP.cov: $(BP_FILES)
synth/BP.vg: $(BP_FILES)

BTB_FILES = verilog/sys_defs.svh
build/BTB.simv: $(BTB_FILES)
build/BTB.cov: $(BTB_FILES)
synth/BTB.vg: $(BTB_FILES)

PHT_FILES = verilog/sys_defs.svh
build/PHT.simv: $(PHT_FILES)
build/PHT.cov: $(PHT_FILES)
synth/PHT.vg: $(PHT_FILES)

RAS_FILES = verilog/sys_defs.svh
build/RAS.simv: $(RAS_FILES)
build/RAS.cov: $(RAS_FILES)
synth/RAS.vg: $(RAS_FILES)

PD_FILES = verilog/sys_defs.svh
build/PD.simv: $(PD_FILES)
build/PD.cov: $(PD_FILES)
synth/PD.vg: $(PD_FILES)
...
FETCH_FILES = verilog/sys_defs.svh verilog/FIFO.sv verilog/michael_id.sv verilog/s_PhysicalRegisterFile.sv
build/if_stage.simv: $(FETCH_FILES)
build/if_stage.cov: $(FETCH_FILES)
synth/if_stage.vg: $(FETCH_FILES)

PRF_FILES = verilog/sys_defs.svh
build/s_PhysicalRegisterFile.simv: $(PRF_FILES)
build/s_PhysicalRegisterFile.cov: $(PRF_FILES)
synth/s_PhysicalRegisterFile.vg: $(PRF_FILES)

EXALU_FILES = verilog/sys_defs.svh
build/if_stage.simv: $(EXALU_FILES)
build/if_stage.cov: $(EXALU_FILES)
synth/if_stage.vg: $(EXALU_FILES)

EXMEM_FILES = verilog/sys_defs.svh
build/if_stage.simv: $(EXMEM_FILES)
build/if_stage.cov: $(EXMEM_FILES)
synth/if_stage.vg: $(EXMEM_FILES)

EXMULT_FILES = verilog/sys_defs.svh
build/if_stage.simv: $(EXMULT_FILES)
build/if_stage.cov: $(EXMULT_FILES)
synth/if_stage.vg: $(EXMULT_FILES)

EXECUTE_FILES = verilog/sys_defs.svh verilog/FIFO.sv
build/if_stage.simv: $(EXECUTE_FILES)
build/if_stage.cov: $(EXECUTE_FILES)
synth/if_stage.vg: $(EXECUTE_FILES)

STAGE_ISSUE_FILES = verilog/sys_defs.svh verilog/FIFO.sv verilog/s_PhysicalRegisterFile.sv verilog/stage_ex.sv verilog/rs.sv
build/stage_issue.simv: $(STAGE_ISSUE_FILES)
build/stage_issue.cov: $(STAGE_ISSUE_FILES)
synth/stage_issue.vg: $(STAGE_ISSUE_FILES)

STAGE_COMPLETE_FILES = verilog/sys_defs.svh verilog/stage_complete.sv
build/stage_issue.simv: $(STAGE_COMPLETE_FILES)
build/stage_issue.cov: $(STAGE_COMPLETE_FILES)
synth/stage_issue.vg: $(STAGE_COMPLETE_FILES)

STAGE_RETIRE_FILES = verilog/sys_defs.svh verilog/stage_retire.sv
...
RAT_FILES = verilog/sys_defs.svh verilog/s_RegisterAliasTable.sv
build/s_RegisterAliasTable.simv: $(RAT_FILES)
build/s_RegisterAliasTable.cov: $(RAT_FILES)
synth/s_RegisterAliasTable.vg: $(RAT_FILES)

ARCHMAP_FILES = verilog/sys_defs.svh verilog/s_ArchitecturalMap.sv
build/s_ArchitecturalMap.simv: $(ARCHMAP_FILES)
build/s_ArchitecturalMap.cov: $(ARCHMAP_FILES)
synth/s_ArchitecturalMap.vg: $(ARCHMAP_FILES)

STAGE_EX_FILES = verilog/sys_defs.svh
build/stage_ex.simv: $(STAGE_EX_FILES)
build/stage_ex.cov: $(STAGE_EX_FILES)
synth/stage_ex.vg: $(STAGE_EX_FILES)

IB_FILES = verilog/sys_defs.svh
build/ib.simv: $(IB_FILES)
build/ib.cov: $(IB_FILES)
synth/ib.vg: $(IB_FILES)

BP2B_FILES = verilog/sys_defs.svh
build/bp2b.simv: $(BP2B_FILES)
build/bp2b.cov:  $(BP2B_FILES)
synth/bp2b.vg:   $(BP2B_FILES)

LOAD_FILES = verilog/sys_defs.svh
build/load.simv: $(LOAD_FILES)
build/load.cov:  $(LOAD_FILES)
synth/load.vg:   $(LOAD_FILES)
...
CPU_SOURCES = verilog/cpu.sv \
			  ...
			  verilog/rob.sv \
			  verilog/FIFO.sv \
			  verilog/s_RegisterAliasTable.sv \
			  verilog/s_ArchitecturalMap.sv \
			  verilog/s_PhysicalRegisterFile.sv \
			  verilog/ib.sv \
			  verilog/dcache.sv \
			  verilog/load.sv\
			  verilog/SQ.sv\
			  verilog/store_pipeline.sv\
			  verilog/bp2b.sv\

			  


# CPU_SOURCES =  						verilog/sys_defs.svh \
# 										verilog/rob.sv \
# 										verilog/stage_retire.sv \
# 										verilog/freeList.sv \
# 										verilog/s_ArchitecturalMap.sv \
...
build/integrated_retirement_pipeline.simv: $(INTEGRATED_RETIREMENT_PIPELINE_FILES)
