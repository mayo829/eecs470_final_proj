...
module icache (
    ...
    input  logic dontallowicachebruh
);
    ...
    icache_mem (
        ...
        .we   (got_mem_data && !icache_change_addr), // && !icache_change_addr <- maybe this
        .waddr(current_index),
    ...
    //assign write_en     = got_mem_data && !dontallowicachebruh;
    assign icache_change_addr = ((current_index != last_index) || (current_tag != last_tag) || misprediction);

    // Set mem tag to zero if we icache_change_addr, and keep resetting while there is
    // a miss_outstanding. Then set to zero when we got_mem_data.
    // (this relies on Imem2proc_transaction_tag being zero when there is no request)
    assign update_mem_tag = icache_change_addr || miss_outstanding || got_mem_data;

    // If we have a new miss or still waiting for the response tag, we might
    // need to wait for the response tag because dcache has priority over icache
    assign unanswered_miss = icache_change_addr ? !Icache_valid_out :
                                        miss_outstanding && (Imem2proc_transaction_tag == 0);

    // Keep sending memory requests until we receive a response tag or change addresses
    assign proc2Imem_command = (miss_outstanding && !icache_change_addr) ? MEM_LOAD : MEM_NONE;
    ...
    always_ff @(posedge clock) begin
        if (reset) begin
            ...
            if (update_mem_tag | misprediction) begin
                current_mem_tag <= Imem2proc_transaction_tag;
            end
            if (got_mem_data && !icache_change_addr) begin // If data came from memory, meaning tag matches //MAYBE NEED THIS && !icache_change_addr
                icache_tags[current_index].tags  <= current_tag;
